 ...
 */
    ...
    foreach($fileList as $fileName) {
        ...
        $anotherCode = $wp_filesystem->get_contents($fileName);

        if ($anotherCode === false) {
            // Handle error
    ...
    // For local file, use WP_Filesystem
    $content = $wp_filesystem->get_contents($versionFile);
    if ($content !== false) {
        $versionDataEx = explode("\n", $content); // Split the content by new lines
    } else {
        // Handle error for local file
        throw new Exception('Failed to read the local version file.');
      }
	//$versionDataEx = explode("\n", file_get_contents($versionFile));
    
    // Use WP_Filesystem to get the contents of the file
    $outputContents = $wp_filesystem->get_contents($headerFile);
    ...
    // Assuming $outputContents is already defined and contains the data to be written
    if ($wp_filesystem->put_contents($outputFile, $outputContents)) {
        // File written successfully
    } else {
        // Handle error
        throw new Exception('Failed to write to the output file.');
    }
