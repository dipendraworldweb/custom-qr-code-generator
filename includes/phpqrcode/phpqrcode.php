 ...
 */
    ...
    class QRtools {
            ...
            if (QR_LOG_DIR !== false) {
                if ($err != '') {
                    ...
                    if ($outfile !== false) {
                        ...
                        $log_file = QR_LOG_DIR . basename($outfile) . '-errors.txt';
                        ...
                        $log_file = QR_LOG_DIR.'errors.txt';
                        $log_content = gmdate('Y-m-d H:i:s') . ': ' . $err;
        ...
        public static function save($content, $filename_path)
        {
            ...
            try {
                // Use WP_Filesystem API to write the file
                if ($wp_filesystem->put_contents($filename_path, $content, FS_CHMOD_FILE)) {
                    return true;
                } else {
                    throw new Exception('Failed to write the file.');
                }
            } catch (Exception $e) {
                echo 'Exception reçue: ', esc_html( $e->getMessage() ), "\n";
            }
            ...
            if(!isset(self::$frames[$version])) {
                ...
                if (QR_CACHEABLE) {
                    if (file_exists($fileName)) {
                        // Use WP_Filesystem API to read the file
                        $content = $wp_filesystem->get_contents($fileName);

                        if ($content !== false) {
                            self::$frames[$version] = self::unserial($content); // Process content
                        } else {
                            // Handle the error
                            throw new Exception('Failed to read the local file.');
                        }
                        //self::$frames[$version] = self::unserial(file_get_contents($fileName)); 
                    } else {
                        self::$frames[$version] = self::createFrame($version);
                        // Use WP_Filesystem API to write the content to the file
                        $content = self::serial(self::$frames[$version]);
 ...
 */
	...
	class QRmask {
        ...
        public function makeMaskNo($maskNo, $width, $s, &$d, $maskGenOnly = false)
        {
            ...
            if (QR_CACHEABLE) {
                if (file_exists($fileName)) {
                    // Use WP_Filesystem to get the contents of the file
                    $content = $wp_filesystem->get_contents($fileName);

                    if ($content !== false) {
                        $bitMask = self::unserial($content); // Unserialize the content
                    } else {
                        // Handle the error
                        throw new Exception('Failed to read the local file.');
                    }
                    ...
                    if (!file_exists(QR_CACHE_DIR.'mask_'.$maskNo))
                        ...
                        $dir = QR_CACHE_DIR . 'mask_' . $maskNo;

                        // Check if the directory exists, if not, create it
                        if (!$wp_filesystem->is_dir($dir)) {
                            $wp_filesystem->mkdir($dir, FS_CHMOD_DIR); // Create directory with appropriate permissions
                        }
                        //mkdir(QR_CACHE_DIR.'mask_'.$maskNo); 
                    // Use WP_Filesystem API to write the serialized bitMask to the file
                    $content = self::serial($bitMask);
 ...
 */
    ...
    class QRencode {
        ...
        public function encode($intext, $outfile = false) 
        {
            ...
            if ($outfile!== false) {
                // Prepare the content to write (joining the binarized data)
                $content = join("\n", QRtools::binarize($code->data));

                // Use WP_Filesystem to write the content to the file
                if ($wp_filesystem->put_contents($outfile, $content)) {
    ...
    class QRvect {
        ...
        public static function eps($frame, $filename = false, $pixelPerPoint = 4, $outerFrame = 4,$saveandprint=FALSE, $back_color = 0xFFFFFF, $fore_color = 0x000000, $cmyk = false) 
        {
            ...
            if ($filename === false) {
                ...
                echo $vect; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            } else {
                if($saveandprint===TRUE){
