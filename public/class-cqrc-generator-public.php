...
class Cqrc_Generator_Public {
	...
	public function enqueue_styles() {
		 ...
		 * class.
		 */
		// Check if the post contains the shortcode '[qrcode]'
		global $post;
		if (has_shortcode($post->post_content, 'cqrc_gen_qrcode_view')) {
			wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/cqrc-generator-public.css', array(), $this->version, 'all' );
		}
	...
	public function enqueue_scripts() {
		...
		global $post;
		if (has_shortcode($post->post_content, 'cqrc_gen_qrcode_view')) {
			wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/cqrc-generator-public.js', array( 'jquery' ), $this->version, array('strategy'  => 'defer', 'in_footer' => true) );
		}
